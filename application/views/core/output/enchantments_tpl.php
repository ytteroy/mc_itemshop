<div class="preview">
    ...
    Mantas vērtība: <strong><span id="item-price"><?php echo $this->module->sms_prices[$item->price_sms_lv]; ?></span> EUR</strong><br />
    Kopā ar uzlabojumiem: <strong><span id="item-upgraded"><?php echo $this->module->sms_prices[$item->price_sms_lv]; ?></span> EUR</strong><br />
    Maksājot ar SMS: <strong><span id="total-smsprice"><?php echo $this->module->sms_prices[$item->price_sms_lv]; ?></span> EUR</strong> <i class="icon-comment-alt blue icon-large" title="Maksājot ar SMS cena var atšķirties no groza cenas, jo tiek sameklēts tuvākais augstākais SMS cenas kods."></i>
        ...
        <div class="controls">
            ...
            <select id="<?php echo $e->cmd; ?>" name="<?php echo $e->cmd; ?>" class="chosen enchantment-menu" data-level-price="<?php echo $e->levelprice_sms_lv; ?>">
                ...
                for ($i = 1; $i <= $e->level_limit; $i++):
                    ...
                    echo '<option value="'.$i.'">+ '.$i.' level ( + '. number_format($e->levelprice_sms_lv * $i / 100, 2, '.', '') .' EUR )</option>';
